 ...
 * Third line of input consists of numbers separated by a space which make up
 * the King's palindrome list.
 * 
...
class KingsPalindromeList {
    ...
    public void run() {
        ...
        String answer = switch (taskNumber) {
            //calling methods accordingly to the input task number and storing answer
            case 1 -> taskOne();
            case 2 -> taskTwo();
            case 3 -> taskThree();
    ...
    /**
     * Method for finding the smallest palindromes greater than or equal
     * to the numbers from the King's list.
     * @return result string containing the correct list of palindromes.
     */
    String taskOne() {
        StringBuilder correctList = new StringBuilder();
        //creating modifiable array that will collect the correct numbers
        for (String original : numbers) {
            //for-loop going through each number from the King's list
            String reversed = new StringBuilder(original).reverse().toString();
            //reversing the number
            if (!reversed.equals(original)) {
                int number = Integer.parseInt(original);
                //convert the number from the list to int data type
                while (!reversed.equals(original)) {
                    //while loop until the number becomes a palindrome
                    number += 1;
            ...
            //add the number to the array with the correct numbers
        }
    ...
    String taskTwo() {
        ...
        //calling first method to convert all numbers to palindromes
        int maxMagicNumber = 1;
        //integer value for storing the maximum length of magic set
        for (String number : numbers) {
            int currentMagicNumber = 1;
            if (number.length() > maxMagicNumber * 2) {
                //checks whether number is longer than already found magic set
    ...
    String taskThree() {
        ...
        //arraylist containing the longest magic set
        for (String number : numbers) {
            ...
            if (number.length() > longestMagicSet.size() * 2) {
                ...
                // the third task requires additional check for the case
                // when there are multiple longest magic sets
                if (currentMagicSet.size() > longestMagicSet.size()) {
                    longestMagicSet = currentMagicSet;
                        ...
                        && Integer.parseInt(currentMagicSet.get(0))
                        > Integer.parseInt(longestMagicSet.get(0))) {
                    longestMagicSet = currentMagicSet;
                }
            }
        }
        Collections.reverse(longestMagicSet);
        // reversing the arraylist to make elements in ascending order
        StringBuilder correctList = new StringBuilder();
        for (String number : longestMagicSet) {
            // this cycle is to build the answer string from elements of arraylist
